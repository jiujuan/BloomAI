...
import type { SkillArtifact, SkillRun, SkillVersion } from './skill-runtime.types'
...
export function RunDetailDrawer({ runId, onClose }: { runId: string; onClose: () => void }) {
  const { selectedRun, runEvents, runArtifacts, loadRun, loadRunEvents, loadArtifacts, commandRun, exportArtifact } = useSkillRuntimeStore()
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const exportRunArtifact = async (artifact: SkillArtifact) => {
    ...
    await exportArtifact(artifact.id, { runId, destinationDir, confirmed: true, auditReason: 'Exported from Run detail' })
  }
