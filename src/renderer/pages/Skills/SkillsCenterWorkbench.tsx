...
import { AlertTriangle, Filter, Puzzle, Search, SlidersHorizontal, Upload } from 'lucide-react'
import { useSkillRuntimeStore } from './skill-runtime.store'
import type { RuntimeErrorScope, SkillArtifact, SkillInstallation, SkillPackage, SkillRun, SkillRuntimeCapabilities, SkillRuntimeFilterStatus, SkillRuntimeSourceFilter, SkillVersion } from './skill-runtime.types'
...
import type { SkillsCenterTab as SkillsRouteTab } from './SkillsSidebar'
...
export function getVisibleSkillsRuntimeError(
  error: string | null | undefined,
  errorScope: RuntimeErrorScope | null | undefined,
  tab: SkillsCenterTab,
) {
  if (!error) return null

  // Import errors are rendered beside the scan/install action, not in the shell.
  if (tab === 'import' || errorScope === 'import') return null

  // A scoped error belongs only to the page that initiated the operation.
  if (errorScope && errorScope !== 'global' && errorScope !== tab) return null

  return shouldHideSkillsAdminAccessError(error) ? null : error
}
...
export function SkillsCenterWorkbench() {
  ...
  const selectTab = (next: SkillsCenterTab) => {
    ...
    setRoute({ tab: nextView, selectedPackageId: nextView === 'detail' ? route.selectedPackageId : undefined })
  }
  ...
  const counts = { center: catalogRows.length, import: packageRows.length, detail: selectedPackage ? 1 : 0, runs: runtime.runs.length, artifacts: artifactRecords.length, settings: runtime.diagnostics ? 1 : 0 }
  ...
  const activeRuntimeError = getVisibleSkillsRuntimeError(runtime.error, runtime.errorScope, tab)
  const panelRuntimeError = tab === 'center' || tab === 'runs' || tab === 'artifacts' ? activeRuntimeError : null
  const shellRuntimeError = panelRuntimeError ? null : activeRuntimeError
  return <div className="skills-center skills-admin-shell skills-runtime-page" data-testid="skills-admin-shell" data-testid-secondary="skills-center-workbench">
    ...
    <div className="skills-center-layout"><SkillsSidebar view={tab} counts={counts} onChange={selectTab} /><main className="skills-center-main">
      ...
      {shellRuntimeError && <div className="skills-page-message"><AlertTriangle size={14} aria-hidden="true" />{shellRuntimeError}<button type="button" onClick={runtime.clearError} aria-label="关闭提示">×</button></div>}
      {tab === 'import' && <PackageImportWorkbench onInstalled={(context) => { if (context.packageId) void openPackage(context.packageId) }} onCancel={() => selectTab('center')} />}
      {tab === 'center' && <SkillOverviewPanel rows={rows} allRows={catalogRows} tab="center" loading={runtime.loading} error={panelRuntimeError} runs={runtime.runs} page={catalogPage} pageSize={SKILLS_CATALOG_PAGE_SIZE} totalRows={catalogTabRows.length} onPageChange={setCatalogPage} onOpenPackage={openPackage} onOpenRun={openRun} onToggleInstallation={toggleInstallation} onUninstallInstallation={uninstallInstallation} onInstall={openImport} catalogSearch={filters.query} catalogSort={catalogSort} catalogTab={catalogTab} catalogFiltersOpen={catalogFiltersOpen} onCatalogSearchChange={handleGlobalSearch} onCatalogSortChange={setCatalogSort} onCatalogTabChange={setCatalogTab} onCatalogFilterClick={() => setCatalogFiltersOpen((current) => !current)} />}
      {tab === 'runs' && <RunsWorkbench runs={runtime.runs} artifactCounts={artifactCounts} loading={runtime.loading} error={panelRuntimeError} onOpenRun={openRun} onRefresh={() => void runtime.loadRuns()} />}
      {tab === 'artifacts' && <ArtifactsWorkbench records={artifactRecords} loading={runtime.loading} error={panelRuntimeError} onOpenRun={openRun} onExport={exportArtifact} />}
      ...
      {tab === 'detail' && selectedPackage && <div className="skills-center-detail-grid"><SkillVersionPanel versions={selectedPackage.versions} currentVersionId={selectedPackage.installations[0]?.currentVersionId || selectedPackage.installations[0]?.current_version_id} selectedVersionId={selectedVersion?.id} onSelect={runtime.selectVersion} /></div>}
    </main></div>
    {selectedPackage && <PackageDetailDrawer detail={selectedPackage} runs={runtime.runs} selectedVersionId={selectedVersion?.id} onSelectVersion={runtime.selectVersion} onClose={() => setRoute((current) => ({ ...current, selectedPackageId: undefined }))} onRun={startRun} onOpenRun={openRun} />}
