    ...
    {visible.map((project) => {
      ...
      return <div key={project.id} className="project-tree-item">
        <div className="project-row-wrap">
          <button type="button" className="project-row" aria-expanded={expanded} aria-label={`${project.name}，${project.sessionCount} 个聊天`} onClick={() => onToggleProject(project.id)}>
            {expanded ? <FolderOpen size={15} /> : <Folder size={15} />}<span className="project-row-name">{project.name}</span>
          </button>
          <button type="button" className="project-new-session" aria-label={`在 ${project.name} 中新建聊天`} title="在此项目中新建聊天" onClick={() => onCreateSession(project.id)}><Plus size={14} /></button>
        </div>
        <ProjectSessions projectId={project.id} expanded={expanded} />
      </div>
    })}
    {projects.length === 0 && <div className="session-empty"><p>还没有项目</p></div>}
    {projects.length > 6 && <button type="button" className="sidebar-more-button" onClick={onToggleProjectList}>{projectListExpanded ? '收起文件夹' : '更多文件夹'}</button>}
  </>
}
