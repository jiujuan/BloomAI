...
export function runBootstrapSql() {
  ...
  // Additive migration: rich UI parts (tool cards, reasoning, workflow steps) for assistant
  // messages. Older DBs created the table without this column, so add it idempotently.
  try { rawDb.exec(`ALTER TABLE messages ADD COLUMN parts TEXT`) } catch { /* column already exists */ }
...
function seedTools() {
  ...
  if (Number(count) < 25) {
    const tools = [
      ...
      ['web_screenshot', 'web', 'Web Screenshot', 'Capture a bounded screenshot of a URL as a controlled PNG or JPEG artifact.', '{"url":{"type":"string"},"fullPage":{"type":"boolean","default":true},"viewport":{"type":"object"},"format":{"type":"string","enum":["png","jpeg"],"default":"png"},"quality":{"type":"number"},"timeoutMs":{"type":"number","default":60000}}', '{"runId":{"type":"string"},"relativePath":{"type":"string"},"mimeType":{"type":"string"},"width":{"type":"number"},"height":{"type":"number"},"bytes":{"type":"number"},"finalUrl":{"type":"string"},"provider":{"type":"string"},"diagnostics":{"type":"object"}}', 'network'],
      ...
      ['fs_apply_patch', 'fs', 'Apply Patch', 'Preview or apply a bounded unified diff inside an approved workspace root.', '{"patch":{"type":"string"},"root":{"type":"string"},"dryRun":{"type":"boolean","default":true},"createBackup":{"type":"boolean","default":true}}', '{"dryRun":{"type":"boolean"},"applied":{"type":"boolean"},"files":{"type":"array"},"modifiedFiles":{"type":"array"},"conflicts":{"type":"array"},"backupPaths":{"type":"array"},"rollbackToken":{"type":"string"}}', 'write'],
  ...
  database.update(schema.tools).set({
    ...
    params_schema: '{"url":{"type":"string"},"mode":{"type":"string","enum":["text","html","full"],"default":"text"},"maxChars":{"type":"number","default":20000},"render":{"type":"boolean","description":"Force JS rendering via a headless browser. Omit for auto (renders only when the static page has little text, e.g. SPAs)."},"timeoutMs":{"type":"number","default":20000}}',
