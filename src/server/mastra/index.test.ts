...
describe('shutdownMastraRuntime', () => {
  it('shuts down Mastra, cached project workspaces, and runtime storage in order', async () => {
    ...
    const mcpConnectionManager = { disconnectAll: vi.fn().mockResolvedValue(undefined) }
    ...
    await shutdownMastraRuntime({ mastra, mcpConnectionManager, projectWorkspaceFactory, scheduleRuntimeStorage } as any)

    expect(mastra.shutdown).toHaveBeenCalledOnce()
    expect(mcpConnectionManager.disconnectAll).toHaveBeenCalledOnce()
    expect(projectWorkspaceFactory.shutdown).toHaveBeenCalledOnce()
    expect(scheduleRuntimeStorage.close).toHaveBeenCalledOnce()
    expect(mastra.shutdown.mock.invocationCallOrder[0]).toBeLessThan(mcpConnectionManager.disconnectAll.mock.invocationCallOrder[0])
    expect(mcpConnectionManager.disconnectAll.mock.invocationCallOrder[0]).toBeLessThan(projectWorkspaceFactory.shutdown.mock.invocationCallOrder[0])
