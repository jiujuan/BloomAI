...
- 在现有导航下增加 MCP Servers 页面，覆盖 Server 列表/详情、配置编辑、新增、软删除、连接测试、Catalog Preview/Diff/Confirm、启用/禁用、Trust Level、Tool Policy、手工 Test、Approval 和 Runs/Audit；
- renderer API 统一走 `API_BASE` 与 `/mcp/...`，Server Safe DTO 只展示 stdio command/args 摘要、HTTP origin 和 Header 名称，Secret 值要求使用 `${env:NAME}` 引用且不会回填；
- Store 对配置变化清除 Preview、Tool、连接测试和旧 Approval 状态，Confirm/Approve/Deny 后重新加载服务端状态，未 Confirm 的 Tool 不做 enabled 乐观更新，过期 Preview 禁止 Confirm；
- Feature Flag 关闭时 fail closed，仅显示安全禁用状态；API、Store 和 UI 测试覆盖 Secret/Header/Approval Token 不泄露、错误提示和安全状态。

Task 9 已通过 `npm run test:mcp-ui`、`npm run typecheck` 和 `npm run build` 验证。
