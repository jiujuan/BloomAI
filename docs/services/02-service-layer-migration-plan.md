...
- 新增 `tool.service.ts`、`skill.service.ts` 与 `skill-package-runtime.service.ts`；分别封装工具权限/执行、Legacy Skill 兼容流程和 Package Runtime 的安装、安装状态、运行、命令、事件及 artifact 用例。
- `tools.ts`、`skills.ts`、`skill-package-runtime.ts` 已收敛为薄 HTTP Adapter：仅保留请求读取、分页、Package Runtime 的 Zod 校验、HTTP 错误适配及 artifact 二进制 `Response`。
- 保持既有协议和状态机：Legacy `/skills/:id/run` 对 Package reference 继续返回 `409 PACKAGE_SKILL_ASYNC_ONLY`；Package Runtime 的 revision、幂等 command、取消和 artifact ownership 流程未重写。
- 补充 Service 单元测试与 Tools/Skills/Package Runtime Route contract 测试，覆盖授权/撤销、执行错误、legacy/package reference 优先级、安装失败、状态冲突、取消 command、artifact 缺失和二进制 artifact 响应。
- 移除 `tools.ts`、`skills.ts`、`skill-package-runtime.ts` 的架构 allowlist；Route 不再直接导入 repository、Package Installer、runtime、artifact store 或 capability broker。
- 已完成验证：定向 Services 测试 3 文件 / 17 测试通过；Tools / Skills HTTP contract 测试 2 文件 / 8 测试通过；`npm test` 70 文件 / 319 测试通过；`npm run typecheck`、`npm run test:architecture`（3 测试）和 `npm run build` 均通过；使用随机端口和独立临时 `DATA_DIR` 的真实 Hono local API smoke 通过，覆盖 health、Tools 授权/撤销、Legacy Skill 运行、Package reference 的 409 保护、Package Run 创建/取消和 artifact 读取。未调用真实外部 Provider；临时 server 和数据目录已清理。Renderer 人工 smoke 仍需在可交互桌面会话中执行。
...
- [x] 实现并迁移 `tool.service.ts`。
- [x] 实现并迁移 `skill.service.ts`。
- [x] 等当前 skill runtime 功能改动稳定后，实现并迁移 `skill-package-runtime.service.ts`。
- [x] 完成 artifact、状态机、权限与取消的回归测试。
